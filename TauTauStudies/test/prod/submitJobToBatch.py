...
def createEventList( stream, sample, directory, firstFile, lastFile):
    ...
    f.write('#!/bin/sh\n\n')
    f.write('export WORKINGDIR="/home/llr/cms/lbianchini/CMSSW_4_2_8_patch7/src/Bianchi/TauTauStudies/test/prod/batch/'+stream+'/'+sample+'"\n')
    ...
    submitToQueue = '/opt/exp_soft/cms/t3/t3submit -q cms -k eo -N '+sample+'_'+str(firstFile)+'-'+str(lastFile)+' /home/llr/cms/lbianchini/CMSSW_4_2_8_patch7/src/Bianchi/TauTauStudies/test/prod/batch/'+stream+'/'+sample+'/job'+'_'+str(firstFile)+'-'+str(lastFile)+'.sh > batch/'+stream+'/'+sample+'/jobID'+'_'+str(firstFile)+'-'+str(lastFile)+'.txt'
    print submitToQueue
