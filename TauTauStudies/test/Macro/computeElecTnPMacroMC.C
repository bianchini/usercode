  ...
  
{
  ...
  TFile fTTb("../prod/ElecTnP/treeElecTnP_WJets-madgraph-PUS4-TnP.root");
  ...
  fullTreeSgn->Draw("mass>>hS",Form("tag_puMCWeight*(%s && mass>%f && mass<%f && mcTrue && tag_genDecay==23*11 && tag_pfRelIso<0.1 && pair_tnpCharge==0 && event_met_pfmet<25 && tag_pt>25)",bin_.c_str(),xLow_,xHigh_));
  float SGNtrue = hS->Integral();
  fullTreeSgn->Draw("mass>>hSP",Form("tag_puMCWeight*(%s && %s>=%f && mass>%f && mass<%f && mcTrue && tag_genDecay==23*11 && tag_pfRelIso<0.1 && pair_tnpCharge==0 && event_met_pfmet<25 && tag_pt>25)",bin_.c_str(),category_.c_str(),cutValue_,xLow_,xHigh_));
  ...
  TFile *templFile = new TFile(Form("dummyTempl_%s_bin%.2f.root",binLabel_.c_str(),binCenter_),"RECREATE");
  
  TTree* fullTreeSgnCutP = fullTreeSgn->CopyTree( Form("(%s>=%f && %s && pair_tnpCharge==0 && event_met_pfmet<25 && mcTrue && tag_genDecay==23*11 && tag_pfRelIso<0.1 && tag_pt>25)",category_.c_str(),cutValue_,bin_.c_str()) );
  TTree* fullTreeSgnCutF = fullTreeSgn->CopyTree( Form("(%s< %f && %s && pair_tnpCharge==0 && event_met_pfmet<25 && mcTrue && tag_genDecay==23*11 && tag_pfRelIso<0.1 && tag_pt>25)",category_.c_str(),cutValue_,bin_.c_str()) );
  ...
  TFile* dummyData = new TFile(("dummyData_"+binLabel_+".root").c_str(),"RECREATE");
  TTree* fullTreeDataCutP = fullTreeData->CopyTree( Form("(%s>=%f && %s  && tag_pfRelIso<0.1 && pair_tnpCharge==0 && event_met_pfmet<25 && (tag_hlt1==1 || tag_hlt2==1) && tag_pt>25)",category_.c_str(),cutValue_,bin_.c_str()) ); 
  TTree* fullTreeDataCutF = fullTreeData->CopyTree( Form("(%s <%f && %s  && tag_pfRelIso<0.1 && pair_tnpCharge==0 && event_met_pfmet<25 && (tag_hlt1==1 || tag_hlt2==1) && tag_pt>25)",category_.c_str(),cutValue_,bin_.c_str()) );
  ...
  TFile* dummyBkg = new TFile(Form("dummyBkg%s_%.2f.root",binLabel_.c_str(),binCenter_),"RECREATE");
  TTree* fullTreeZttCutP = fullTreeZtt->CopyTree( Form("(%s>=%f && %s  && tag_pfRelIso<0.1 && pair_tnpCharge==0 && event_met_pfmet<25 && tag_pt>25 && tag_genDecay==23*15 )",category_.c_str(),cutValue_,bin_.c_str()) ); 
  TTree* fullTreeWenCutP = fullTreeWen->CopyTree( Form("(%s>=%f && %s  && tag_pfRelIso<0.1 && pair_tnpCharge==0 && event_met_pfmet<25 && tag_pt>25)",category_.c_str(),cutValue_,bin_.c_str()) ); 
  TTree* fullTreeTTbCutP = fullTreeTTb->CopyTree( Form("(%s>=%f && %s  && tag_pfRelIso<0.1 && pair_tnpCharge==0 && event_met_pfmet<25 && tag_pt>25)",category_.c_str(),cutValue_,bin_.c_str()) ); 
  //
  TTree* fullTreeZttCutF = fullTreeZtt->CopyTree( Form("(%s< %f && %s  && tag_pfRelIso<0.1 && pair_tnpCharge==0 && event_met_pfmet<25 && tag_pt>25 && tag_genDecay==23*15 )",category_.c_str(),cutValue_,bin_.c_str()) ); 
  TTree* fullTreeWenCutF = fullTreeWen->CopyTree( Form("(%s< %f && %s  && tag_pfRelIso<0.1 && pair_tnpCharge==0 && event_met_pfmet<25 && tag_pt>25)",category_.c_str(),cutValue_,bin_.c_str()) ); 
  TTree* fullTreeTTbCutF = fullTreeTTb->CopyTree( Form("(%s< %f && %s  && tag_pfRelIso<0.1 && pair_tnpCharge==0 && event_met_pfmet<25 && tag_pt>25)",category_.c_str(),cutValue_,bin_.c_str()) ); 
  ...
  cPass->SaveAs(Form("./ElecTnP/%s_%.2f.png",fileNameP.c_str(), binCenter_));
  ...
  cFail->SaveAs(Form("./ElecTnP/%s_%.2f.png",fileNameF.c_str(), binCenter_));
	  ...
	  )
{
  ...
  std::ofstream out(("./ElecTnP/elecTnP_"+category_+"_"+binLabel_+".txt").c_str());
  out.precision(4);
  ...
  vector<float*> bin2 = simFit(tnp_,category_,binLabel_,cutValue_,"(abseta>1.5 && "+ptRange_+")", 1.90,0.4, xLow_,xHigh_,nBins_,doBinned_,deltaAlpha_,deltaN_,scale_,mcLumi_,doMC_);
  ...
  gPad->SaveAs( ("./ElecTnP/efficiencyElecTnP"+category_+"_"+binLabel_+".png").c_str() );
