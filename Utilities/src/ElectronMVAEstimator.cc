...
double ElectronMVAEstimator::mva(const reco::GsfElectron& myElectron, int nvertices )  {
  ...
  // if not available, it might be that we are dealing with a patElectron, in that try to cast it
  // in the current default PAT, KF embedding is broken. See:
  // https://hypernews.cern.ch/HyperNews/CMS/get/physTools/2670.html
  if (!validKF) {
    const pat::Electron * myPatElectron = dynamic_cast<const pat::Electron *>(&myElectron);
    if(myPatElectron) {

      myTrackRef = myPatElectron->closestCtfTrackRef();
      validKF = (myTrackRef.isAvailable());
    }
  }
