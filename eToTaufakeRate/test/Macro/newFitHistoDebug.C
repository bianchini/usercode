...
vector<Double_t*> simFit(TFile *outFile_        = 0,
  ...
  //TFile fmix("/data_CMS/cms/lbianchini/35pb/Htt/testNewWriteFromPAT_soup_Htt.root");
  ...
  TTree* fullTreeQcdCutF = fullTreeQcd->CopyTree( Form("(%s<%f && %s)",category_.c_str(),cutValue_,bin_.c_str()) );
  //TTree* fullTreeQcdCutP = fullTreeQcd->CopyTree( Form("(tauAntiEMVA>=0.5 && %s)",bin_.c_str()) );
  TTree* fullTreeWenCutP = fullTreeWen->CopyTree( Form("(%s>=%f && %s)",category_.c_str(),cutValue_,bin_.c_str()) );
  TTree* fullTreeWenCutF = fullTreeWen->CopyTree( Form("(%s<%f && %s)",category_.c_str(),cutValue_,bin_.c_str()) );
  TTree* fullTreeZttCutP = fullTreeZtt->CopyTree( Form("(%s>=%f && %s)",category_.c_str(),cutValue_,bin_.c_str()) );
  TTree* fullTreeZttCutF = fullTreeZtt->CopyTree( Form("(%s<%f && %s)",category_.c_str(),cutValue_,bin_.c_str()) );
  TTree* fullTreeTTbCutP = fullTreeTTb->CopyTree( Form("(%s>=%f && %s)",category_.c_str(),cutValue_,bin_.c_str()) );
  TTree* fullTreeTTbCutF = fullTreeTTb->CopyTree( Form("(%s<%f && %s)",category_.c_str(),cutValue_,bin_.c_str()) );
  TTree* fullTreeSgnFakeCutP = fullTreeSgn->CopyTree( Form("(!mcTrue && %s>=%f && %s)",category_.c_str(),cutValue_,bin_.c_str()) );
  TTree* fullTreeSgnFakeCutF = fullTreeSgn->CopyTree( Form("(!mcTrue && %s<%f && %s)",category_.c_str(),cutValue_,bin_.c_str()) );
  // using selection not presetn in LS samples 
  ...
  float expWenP = fullTreeWenCutP->GetEntries()/(readEventsWen/(7899.*1.32));
  float expWenF = fullTreeWenCutF->GetEntries()/(readEventsWen/(7899.*1.32));
  ...
  float expTTbP = fullTreeTTbCutP->GetEntries()/(readEventsTTb/(94.*1.75));
  float expTTbF = fullTreeTTbCutF->GetEntries()/(readEventsTTb/(94.*1.75));
  ...
  RooDataSet qcdDataSetF("qcdDataSetF","dataset for qcd", RooArgSet(mass), Import( *fullTreeLSCutP ) );
  RooDataHist qcdDataHistF("bkgDataHistF","",RooArgSet(mass),qcdDataSetF, 1.0);
  ...
  RooDataSet wenDataSetF("wenDataSetF","dataset for Wenu", RooArgSet(mass), Import( *fullTreeWenCutF ) );
  ...
  RooDataSet zttDataSetF("zttDataSetF","dataset for Ztautau", RooArgSet(mass), Import( *fullTreeZttCutF ) );
  ...
  RooDataSet ttbDataSetF("ttbDataSetF","dataset for TT~", RooArgSet(mass), Import( *fullTreeTTbCutF ) );
  ...
  RooDataSet sgnFakeDataSetF("sgnFakeDataSetF","dataset for fakes in Z->ee", RooArgSet(mass), Import( *fullTreeSgnFakeCutF ) );
  RooDataHist sgnFakeDataHistF("sgnFakeDataHistF","",RooArgSet(mass),sgnFakeDataSetF, 1.0);
  ...
  //DataDataSetP.plotOn(DataFrameP);
