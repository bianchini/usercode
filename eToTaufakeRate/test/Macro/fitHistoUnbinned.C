...
vector<Double_t*> simFit(TFile *outFile_        = 0,
  ...
  RooDataSet templateP("templateP","dataset for signal-pass template", RooArgSet(mass,mcTrue,tauAntiEMVA,signalPFChargedHadrCands,abseta,matchedID), Import( *fullTreeSgn ), Cut(Form("(mcTrue && %s>=%f && %s)",category_.c_str(),cutValue_,bin_.c_str()) ) );
  ...
  RooDataSet McCombData("McCombData","combined data",mass,Index(category),Import("pass", McDataP) ,Import("fail", McDataF)) ;
  ...
  // convolve the template with a resolution model
  RooGaussian DataResolModF("DataResolModF","",mass,DataMeanResP,DataSigmaResP);
  ...
  RooDataSet DataCombData("DataCombData","combined data",mass,Index(category),Import("pass", DataDataP ) ,Import("fail",DataDataF) ) ;
