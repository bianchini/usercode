...
vector<Double_t*> simFit(TFile *outFile_        = 0,
  ...
  TFile fsup("/data_CMS/cms/lbianchini/35pb/testNewWriteFromPAT_soup_tauAntiEMVA_PILEUP.root");
  ...
  TFile *McP = new TFile("dummyBkg.root","RECREATE");
  TTree* fullTreeBkgCut = fullTreeBkg->CopyTree(  Form("(%s>=%f && %s)",category_.c_str(),cutValue_,bin_.c_str())  );
  ...
  RooCBShape bkgPdf_C("bkgPdf_C","",mass,m1_C,sigma1_C,alfa1_C,n_C);
  RooFitResult* fitRes_C = bkgPdf_C.fitTo(bkgDataSet, Minos(1), Save(1), NumCPU(4) );
  RooArgSet fitParam_C(fitRes_C->floatParsFinal());
  RooRealVar* m1_CFit     = (RooRealVar*)(&fitParam_C["m1_C"]);
  ...
  RooRealVar McMP("McMP","", m1_CFit->getVal(),45,65 );
  RooRealVar McSigmaP("McSigmaP","",13,5,30);
  RooRealVar McAlphaP("McAlphaP","",alfa1_CFit->getVal(),-10.,0.);
  RooRealVar McNP("McNP","n", n_CFit->getVal(), 0,10);
  RooCBShape McBackgroundPdfP("McBackgroundPdfP","",mass,McMP,McSigmaP,McAlphaP,McNP);
  ...
  RooRealVar DataMP("DataMP","",m1_CFit->getVal()*(1+deltaM_),45,65);
  RooRealVar DataSigmaP("DataSigmaP","",13,5,30);
  RooRealVar DataAlphaP("DataAlphaP","",alfa1_CFit->getVal(),-10.,0.);
  RooRealVar DataNP("DataNP","n", n_CFit->getVal(), 0,10);
  RooCBShape DataBackgroundPdfP("DataBackgroundPdfP","",mass,DataMP,DataSigmaP,DataAlphaP,DataNP);
...
void makePlot(const string tnp_      = "etoTauMargLooseNoCracks70",
  ...
  leg->AddEntry(h1mcTruth,"MC-truth","lep");
  leg->AddEntry(h1tnpMC,"t&p: simulation (L_{equiv}=500 pb^{-1})","lep");
  leg->AddEntry(h1tnpDATA,"t&p: 7 TeV Data","lep");
  leg->Draw();
