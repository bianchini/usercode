...
vector<Double_t*> simFit(TFile *outFile_        = 0,
  ...
  RooRealVar leadPFChargedHadrCandTrackPt("leadPFChargedHadrCandTrackPt","",0,100);
  RooRealVar leadPFCandPt("leadPFCandPt","",0,100);
  RooRealVar electronPreIDOutput("electronPreIDOutput","",-1000,1.0);
  RooRealVar abseta("abseta","",0,10);
  RooRealVar pt("pt","",15,200);
  ...
  //template from data: events passing tag:WP70 and probe: mdeium iso && WP95
  RooDataSet templatePFromDataLeft("templatePFromDataLeft","dataset for signal-pass template from Data", RooArgSet(mass,matchedID,abseta,electronPreIDOutput), Import( *fullTreeDataForTemplate ), Cut(Form("(matchedID<=0.95 && electronPreIDOutput<0.1 && %s)",bin_.c_str()) ) );
  RooDataSet templatePFromDataRight("templatePFromDataRight","dataset for signal-pass template from Data", RooArgSet(mass,abseta,tauAntiEMVA), Import( *fullTreeDataForTemplate ), Cut(Form("(tauAntiEMVA>0.5 && %s)",bin_.c_str()) ) );
  
  //template from data: events passing tag:W70 and probe: medium iso
  ...
  RooKeysPdf passingPdfL("passingPdfL","",mass,templatePFromDataLeft);
  RooKeysPdf passingPdfR("passingPdfR","",mass,templatePFromDataRight);

  // reduce bin size
  mass.setBins( 12 );
  RooDataHist templateHistPHighPt("templateHistPHighPt","",RooArgSet(mass), templatePFromDataHighPt, 1.0);
  RooHistPdf TemplateSignalPdfP("TemplateSignalPdfP","",mass,templateHistPHighPt);
  ...
  //RooAddPdf TemplateSignalPdfP("TemplateSignalPdfP","",RooArgList(pdfLeft,pdfRight),RooArgList(NLeft,NRight));

 
  RooDataSet templateF("templateF","dataset for signal-pass template", RooArgSet(mass,mcTrue,tauAntiEMVA,signalPFChargedHadrCands,abseta,matchedID), Import( *fullTreeSgn ), Cut(Form("(mcTrue && %s<%f && %s)",category_.c_str(),cutValue_,bin_.c_str()) ));
  ...
  templatePFromDataRight.plotOn(TemplateFrameP);
  ...
  RooDataHist McCombData("McCombData","combined data",mass,Index(category),Import("pass", *(McDataP.createHistogram("histoMcP",mass))) ,Import("fail", *(McDataF.createHistogram("histoMcF",mass)))) ;
  ...
  RooFitResult* ResDataCombinedFit =  DataSimPdf.fitTo(DataCombData , Extended(1), Minos(1), Save(1), NumCPU(4) );
  outFile_->cd(Form("bin%.2f",binCenter_));
...
void makePlot(const string tnp_      = "etoTauMargLooseNoCracks70",
  ...
  vector<Double_t*> bin1Results = simFit(outFile,tnp_, category_,cutValue_,
					 string(Form("%s>%f && %s<%f",var_.c_str(),bin1_,var_.c_str(),bin2_)),
					 (bin2_+bin1_)/2,(bin2_-bin1_)/2 
					 ,xLow_, xHigh_, nBins_);

  cout << "******************** bin2" << endl;
  vector<Double_t*> bin2Results = simFit(outFile,tnp_, category_,cutValue_,
					 string(Form("(%s > %f && %s<%f)",var_.c_str(),bin2_,var_.c_str(),bin3_)),
