...
vector<Double_t*> simFit(bool makeSoupFit_ = false,
  ...
  w->factory("RooFFTConvPdf::McSignalPdfP(mass,TemplateSignalPdfP,RooGaussian::McResolModP(mass,McMeanResP[0.0,-5.,5.],McSigmaResP[0.5,0.,10]))");
  w->factory("RooFFTConvPdf::McSignalPdfF(mass,TemplateSignalPdfF,RooGaussian::McResolModF(mass,McMeanResF[0.0,-5.,5.],McSigmaResF[0.5,0.,10]))");
  ...
  // try to constrain the Nbkg fail
  //w->factory(Form("RooGaussian::ConstrainMcNumBkgF(McNumBkgF,McNumBkgFMean[%f],McNumBkgFSigma[%f]))",BKGFail,0.5*BKGFail));
