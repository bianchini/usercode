...

{
  ...
  // data
  TFile fdat("/lustre/cms/store/user/calabria/Data/TagAndProbe/January_2012_4/SingleElectron_Run2011B_PromptReco1/testTagAndProbe_SingleEleRun2011B.root");
  TTree *fullTreeData = (TTree*)fdat.Get((tnp_+"/fitter_tree").c_str());

  TH1F* hS           = new TH1F("hS","",1,0,150);
  TH1F* hSP          = new TH1F("hSP","",1,0,150);

  fullTreeSgn->Draw("mass>>hS",Form("tag_puMCWeight2011B*(%s && mass>%f && mass<%f && mcTrue && pair_charge==0 && event_met_pfmet<25)",bin_.c_str(),xLow_,xHigh_));
  double SGNtrue = hS->Integral();
  fullTreeSgn->Draw("mass>>hSP",Form("tag_puMCWeight2011B*(%s && %s>=%f && mass>%f && mass<%f && mcTrue && pair_charge==0 && event_met_pfmet<25)",bin_.c_str(),category_.c_str(),cutValue_,xLow_,xHigh_));
  ...
  //Tight
  //RooRealVar meanBkg("meanBkg","",70,66,75);
  //RooRealVar sigmaBkg("sigmaBkg","",5,0,35);

  //MVA
  RooRealVar meanBkg("meanBkg","",70,65,80);
  RooRealVar sigmaBkg("sigmaBkg","",10,0,35);
  ...
  fullTreeSgnCutF->Draw("mass>>hmassSgnF", "tag_puMCWeight2011B");
  ...
  fullTreeSgnCutP->Draw("mass>>hmassSgnP", "tag_puMCWeight2011B");
  mass.setBins( 120 );
  ...
  //Medium
  //RooRealVar meanBkg1("meanBkg1","",90,85,95);
  //RooRealVar sigmaBkg1("sigmaBkg1","",20,0,35);

  //Tight
  //RooRealVar meanBkg1("meanBkg1","",85,70,90);
  //RooRealVar sigmaBkg1("sigmaBkg1","",15,0,30);
  ...
  //RooRealVar np("np","",0.5,0,20);
  //RooCBShape bkgPdfF("bkgPdfF","",mass,meanBkg1,sigmaBkg1,alpha,np);
  RooRealVar meanBkg1("meanBkg1","",85,70,95);
  RooRealVar sigmaBkg1("sigmaBkg1","",15,0,30);
  RooLandau bkgPdfF("bkgPdfF","",mass,meanBkg1,sigmaBkg1); //Landau
...

{
  ...
  //Loose
  //RooRealVar meanBkg("meanBkg","",90,80,95);
  //RooRealVar sigmaBkg("sigmaBkg","",30,0,30);

  //Medium
  //RooRealVar meanBkg("meanBkg","",80,70,90);
  //RooRealVar sigmaBkg("sigmaBkg","",10,0,25);

  //Tight
  //RooRealVar meanBkg("meanBkg","",70,60,80);
  //RooRealVar sigmaBkg("sigmaBkg","",10,0,35);
  ...
  //Tight
  //RooRealVar meanBkg1("meanBkg1","",90,60,95);
  //RooRealVar sigmaBkg1("sigmaBkg1","",15,0,35);
  //RooLandau bkgPdfF("bkgPdfF","",mass,meanBkg1,sigmaBkg1); //Landau
...
// MVA 20,23
  ...
  vector<double*> bin2 = simFit2(tnp_,category_,cutValue_,"abseta>1.5",1.90,0.4,50,120,20,doBinned_,deltaAlpha_,deltaN_,scale_);
  ...
  double truthMC_x[2]   = {0.75, (bin2[0])[0]};
  double truthMC_xL[2]  = {0.75, (bin2[0])[1]};
  double truthMC_xH[2]  = {0.75, (bin2[0])[2]};
  double truthMC_y[2]   = {0.0148622, (bin2[0])[3]};
  double truthMC_yL[2]  = {0.000101762, (bin2[0])[4]};
  double truthMC_yH[2]  = {0.000101762, (bin2[0])[5]};

  double tnpData_x[2]   = {0.75, (bin2[1])[0]};
  double tnpData_xL[2]  = {0.75, (bin2[1])[1]};
  double tnpData_xH[2]  = {0.75, (bin2[1])[2]};
  double tnpData_y[2]   = {0.0115326, (bin2[1])[3]};
  double tnpData_yL[2]  = {0.000699841, (bin2[1])[4]};
  double tnpData_yH[2]  = {0.000718863, (bin2[1])[5]};
