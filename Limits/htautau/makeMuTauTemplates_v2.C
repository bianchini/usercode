...
void produce(   
  ...
  in.open(Form("templates/muTau_%s_template_v2.txt",     binNameSpace.c_str()));
  ...
  while (in.good())
	...
	if(line.find("observation")!=string::npos){
	  ...
	  if(binNameSpace.find("SM0")!=string::npos) 
	    out <<          string(Form("%.2f",1+TMath::Max(fabs(-1+VHrelDown), fabs(-1+VHrelUp) ) * TMath::Abs(-1+VHrelUp)/(-1+VHrelUp)   )); // VH
	  else
	    out << space << string(Form("%.2f",1+TMath::Max(fabs(-1+SMrelDown), fabs(-1+SMrelUp))  * TMath::Abs(-1+SMrelUp)/(-1+SMrelUp)   )); // VH = GGF
	  out << space << string(Form("%.2f",1+TMath::Max(  fabs(-1+SMrelDown), fabs(-1+SMrelUp))  * TMath::Abs(-1+SMrelUp)/(-1+SMrelUp)   ));   // GGF
	  out << space << string(Form("%.2f",1+TMath::Max(  fabs(-1+VBFrelDown),fabs(-1+VBFrelUp))* TMath::Abs(-1+VBFrelUp)/(-1+VBFrelUp) ));   // VBF
	  ...
	  out << space << string(Form("%.2f",1+TMath::Max(  fabs(-1+TTrelDown),fabs(-1+TTrelUp))  * TMath::Abs(-1+TTrelUp)/(-1+TTrelUp) ));  // TT
	  if(binNameSpace.find("SM0")!=string::npos) 
	    out << space << string(Form("%.2f",1+TMath::Max(fabs(-1+VVrelDown),fabs(-1+VVrelUp) ) * TMath::Abs(-1+VVrelUp)/(-1+VVrelUp)));   // VV
	  else
	    out << space << string(Form("%.2f",1+TMath::Max(fabs(-1+TTrelDown),fabs(-1+TTrelUp))  * TMath::Abs(-1+TTrelUp)/(-1+TTrelUp) ));  // TT
