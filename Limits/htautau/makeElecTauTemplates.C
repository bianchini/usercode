...
void produce(   
  ...
  TFile* fin = new TFile(Form("histograms/eTau_mH%d_%s_%s_%s.root", mH_, bin_.c_str() , analysis_.c_str(), variable_.c_str()), "READ");
  ...
  TFile* fTemplOut = new TFile(Form("datacards/eleTauSM_%s.root",variable_.c_str()),"UPDATE");
  ...
  in.open(Form("templates/eleTau_%s_template.txt",  binNameSpace.c_str()));
  ofstream out(Form("datacards/eTau_%s_mH%d.txt", binNameSpace.c_str(), mH_));
  out.precision(8);
  ...
  while (in.good())
	...
	if(line.find("observation")!=string::npos){
	  ...
	  out << line << endl;
          ...
          line.replace( line.find("XXX") , 3 , string(Form("eleTauSM_%s",variable_.c_str()))  );
	  out << line << endl;
	}
	...
	else if(line.find(Form("CMS_htt_eleTau_%s_QCDNorm", binNameSpace.c_str()))!=string::npos){
	...
	else if(line.find(Form("CMS_htt_eleTau_%s_WNorm",binNameSpace.c_str()))!=string::npos && binNameSpace!="SM2"){
...
void produceAll(){
  ...
  produce(120,"diTauVisMass", "ElecUp"  , "vbf");
  produce(120,"diTauVisMass", "ElecDown", "vbf");
  ...
  produce(120,"diTauVisMass", "ElecUp"  , "novbf");
  produce(120,"diTauVisMass", "ElecDown", "novbf");
