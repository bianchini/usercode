			      ...
			      )
{
  ...
  jsonFile[4] = dirJson+"/Cert_190456-208686_8TeV_PromptReco_Collisions12_JSON.txt"; // PromptRecov1 RunD
...
//       if((*jets)[l].Pt()>MINPt1 && TMath::Abs((*jets)[l].Eta())<MAXEta && (*jetPUWP)[l*3]>MINJetID)
    ...
    if( isData && !sample.Contains("Emb") ){
      ...
      bool b_HLTx = false;
      for(int i=0 ; i<9 ; i++)
	b_HLTx = b_HLTx || (*triggerBits)[i]; // 

      HLTx = b_HLTx ? 1.0 : 0.0 ;

      bool b_HLTxQCD = false;
      for(int i=9 ; i<18 ; i++)
	b_HLTxQCD = b_HLTxQCD || (*triggerBits)[i]; // 
	...
	bool isTriggMatched = (*tauXTriggers)[1] && (*tauXTriggers)[2] ; 
	HLTmatch = isTriggMatched ? 1.0 : 0.0;
      }
      else {
	HLTx = HLTmatch = 1.0;
