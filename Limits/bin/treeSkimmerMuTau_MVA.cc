...
void makeTrees_MuTauStream(string analysis_ = "", string sample_ = "", float xsec_ = 0., string inputDir_ = "./"){
  ...
  Float_t PTL2, PTL1, PTL1oPTL2, COSDPHIL1L2, ETAL1, ETAL2, DXY1,DXY2,
    MET, DITAUVISPT, DITAUVISMASS, DITAUVISETA, 
    MTLEG1, PZETAMISS, PZETAVIS,
    DITAUSVFITMASS;
  ...
  for(unsigned int k = 0 ; k < mHMVAs.size(); k++){
    for(unsigned int j = 0 ; j < bkgMVAs.size(); j++){
      ...
      if(bkgMVAs[j].find("AntiZ")!=string::npos){
	...
	reader->BookMVA( "BDT", Form("../../TauTauStudies/test/Macro/tmva/weights/TMVAClassificationPtOrd_MuTau_mH%d_Z_wSVFit_nTrees400_maxDepth3_BDT.weights.xml",mHMVAs[k]));  
	readers.insert(  make_pair(Form("%d%s",mHMVAs[k],bkgMVAs[j].c_str()), reader));
      }
      else if(bkgMVAs[j].find("AntiW")!=string::npos){
	reader->AddVariable( "diTauSVFitMass", &DITAUSVFITMASS);
	reader->AddVariable( "cos(dPhiL1L2)",  &COSDPHIL1L2);
	reader->AddVariable( "ptL2",           &PTL2);
	reader->AddVariable( "ptL1/ptL2",      &PTL1oPTL2);
  ...
  outTreePtOrd->Branch("MVAmH110AntiW",   &MVAmH110AntiW,"MVAmH110AntiW/F");
  outTreePtOrd->Branch("MVAmH115AntiW",   &MVAmH115AntiW,"MVAmH115AntiW/F");
  outTreePtOrd->Branch("MVAmH120AntiW",   &MVAmH120AntiW,"MVAmH120AntiW/F");
  outTreePtOrd->Branch("MVAmH125AntiW",   &MVAmH125AntiW,"MVAmH125AntiW/F");
  outTreePtOrd->Branch("MVAmH130AntiW",   &MVAmH130AntiW,"MVAmH130AntiW/F");
  outTreePtOrd->Branch("MVAmH135AntiW",   &MVAmH135AntiW,"MVAmH135AntiW/F");
  outTreePtOrd->Branch("MVAmH140AntiW",   &MVAmH140AntiW,"MVAmH140AntiW/F");
  outTreePtOrd->Branch("MVAmH145AntiW",   &MVAmH145AntiW,"MVAmH145AntiW/F");
  outTreePtOrd->Branch("MVAmH110AntiZ",   &MVAmH110AntiZ,"MVAmH110AntiZ/F");
  outTreePtOrd->Branch("MVAmH115AntiZ",   &MVAmH115AntiZ,"MVAmH115AntiZ/F");
  outTreePtOrd->Branch("MVAmH120AntiZ",   &MVAmH120AntiZ,"MVAmH120AntiZ/F");
  outTreePtOrd->Branch("MVAmH125AntiZ",   &MVAmH125AntiZ,"MVAmH125AntiZ/F");
  outTreePtOrd->Branch("MVAmH130AntiZ",   &MVAmH130AntiZ,"MVAmH130AntiZ/F");
  ...
  for (int n = 0; n < nEntries ; n++) {
    ...
    (*metsig)[0][0] = 1;//(*metSgnMatrix)[0]; 
    (*metsig)[0][1] = 0;//(*metSgnMatrix)[1]; 
    (*metsig)[1][0] = 0;//(*metSgnMatrix)[1]; 
    (*metsig)[1][1] = 1;//(*metSgnMatrix)[2]; 
    ...
    DITAUSVFITMASS   = diTauSVFitMass;
    COSDPHIL1L2      = TMath::Cos(dPhiL1L2);
    DITAUVISETA      = diTauVisEta;
    PTL2             = ptL2;
    DITAUVISPT       = diTauVisPt;
    PTL1oPTL2        = ptL1/ptL2;
    ETAL1            = etaL1;
    ETAL2            = etaL2;
    MET              = MEt;
    DITAUVISMASS     = diTauVisMass;
    MTLEG1           = MtLeg1_;
    PZETAVIS         = pZeta_;
    PZETAMISS        = (pZeta_-pZetaVis_);
    DXY1             = dxy1; 
    DXY2             = dxy2; 

    MVAmH110AntiW    = readers["110AntiW"]!=0 ? readers["110AntiW"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH115AntiW    = readers["115AntiW"]!=0 ? readers["115AntiW"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH120AntiW    = readers["120AntiW"]!=0 ? readers["120AntiW"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH125AntiW    = readers["125AntiW"]!=0 ? readers["125AntiW"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH130AntiW    = readers["130AntiW"]!=0 ? readers["130AntiW"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH135AntiW    = readers["135AntiW"]!=0 ? readers["135AntiW"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH140AntiW    = readers["140AntiW"]!=0 ? readers["140AntiW"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH145AntiW    = readers["145AntiW"]!=0 ? readers["145AntiW"]->EvaluateMVA( "BDT" ) : -99;

    MVAmH110AntiZ    = readers["110AntiZ"]!=0 ? readers["110AntiZ"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH115AntiZ    = readers["115AntiZ"]!=0 ? readers["115AntiZ"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH120AntiZ    = readers["120AntiZ"]!=0 ? readers["120AntiZ"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH125AntiZ    = readers["125AntiZ"]!=0 ? readers["125AntiZ"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH130AntiZ    = readers["130AntiZ"]!=0 ? readers["130AntiZ"]->EvaluateMVA( "BDT" ) : -99;
    MVAmH135AntiZ    = readers["135AntiZ"]!=0 ? readers["135AntiZ"]->EvaluateMVA( "BDT" ) : -99;
...
void doAllSamplesMu(string inputDir_ = "MuTauStreamFall11_06Dec2011")
{
  ...
  //makeTrees_MuTauStream("TauUp",        samples[18], crossSec[18], inputDir_);
  //makeTrees_MuTauStream("",        samples[26], crossSec[26], inputDir_);
  return;
