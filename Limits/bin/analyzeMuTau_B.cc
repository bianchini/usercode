...
{   
  ...
  */
  ...
  TCut sbin; TCut sbinEmbedding; TCut sbinEmbeddingPZetaRel; TCut sbinPZetaRel; TCut sbinSS; TCut sbinPZetaRelSS; TCut sbinPZetaRev; TCut sbinPZetaRevSS; TCut sbinSSaIso;
  ...
  if(dataLooseIso)
    samples.push_back("DataLooseIso");
  ...
  tMap["Data"]         = data;
  tMap["DataLooseIso"] = dataLooseIso;
  ...
  for( unsigned iter=0; iter<samples.size(); iter++){
	...
	else if((it->first).find("DataLooseIso")!=string::npos){
	  currentTree->Draw(variable_+">>"+h1Name,     "(sampleWeight*puWeight*HLTweightTau*HLTweightMu*SFMu*SFTau*HqTWeight)"*sbinSSaIso);
	  hLooseIsoKeys = new TH1Keys("hLooseIsoKeys","Loose Iso smoothed", nBins , bins.GetArray());
	  currentTree->Draw(variable_+">>hLooseIsoKeys", "(sampleWeight*puWeight*HLTweightTau*HLTweightMu*SFMu*SFTau*HqTWeight)"*sbinSSaIso);
    ...
    if( (it->first).find("DataLooseIso")!=string::npos ) {
  ...
  delete hQCD; delete hZmm; delete hZmj; delete hZfakes; delete hTTb; delete hZtt; delete hW; delete hW3Jets; delete hLooseIso;
