...
void makeTrees_MuTauStream(string analysis_ = "", string sample_ = "", float xsec_ = 0., string inputDir_ = "./", string dirOut_ = "./", int iJson_=-1){
  ...
  TString outName = dirOut_+"/nTuple"+sample+"_Open_MuTauStream_"+analysisFileName+".root";
  ...
  for (int n = 0; n < nEntries ; n++) {
    ...
    if( isData ){
      HLTx = float((*triggerBits)[0] || (*triggerBits)[1] || 
		   ...
		   (*triggerBits)[8]);  
      //HLT_IsoMu18_eta2p1_LooseIsoPFTau20_v4, HLT_IsoMu18_eta2p1_LooseIsoPFTau20_v5, HLT_IsoMu18_eta2p1_LooseIsoPFTau20_v6, HLT_IsoMu17_eta2p1_LooseIsoPFTau20_v2 - HLT_IsoMu17_eta2p1_LooseIsoPFTau20_v7
      HLTxQCD = float((*triggerBits)[9] || (*triggerBits)[10] ||  
		      ...
		      (*triggerBits)[17]);
      //HLT_Mu18_eta2p1_LooseIsoPFTau20_v4, HLT_Mu18_eta2p1_LooseIsoPFTau20_v5, HLT_Mu18_eta2p1_LooseIsoPFTau20_v6, HLT_Mu17_eta2p1_LooseIsoPFTau20_v2 - HLT_Mu17_eta2p1_LooseIsoPFTau20_v7
	...
	if(TMath::Abs((*diTauLegsP4)[0].Eta())<=0.8)
	  SFMu  = ratioMuIDBL1->Eval( (*diTauLegsP4)[0].Pt() ) * ratioMuIsoBL1->Eval( (*diTauLegsP4)[0].Pt() ); 
	else if(TMath::Abs((*diTauLegsP4)[0].Eta())>0.8 && TMath::Abs((*diTauLegsP4)[0].Eta())<=1.2)
	  SFMu  = ratioMuIDBL2->Eval( (*diTauLegsP4)[0].Pt() ) * ratioMuIsoBL2->Eval( (*diTauLegsP4)[0].Pt() );
	else 
	  SFMu  = ratioMuIDEC->Eval( (*diTauLegsP4)[0].Pt() ) * ratioMuIsoEC->Eval( (*diTauLegsP4)[0].Pt() ) ; 
	if(TMath::Abs((*diTauLegsP4)[0].Eta())<=0.8)
          SFMuID = ratioMuIDBL1->Eval( (*diTauLegsP4)[0].Pt() ) ;  
	else if(TMath::Abs((*diTauLegsP4)[0].Eta())>0.8 && TMath::Abs((*diTauLegsP4)[0].Eta())<=1.2)
	  SFMuID = ratioMuIDBL2->Eval( (*diTauLegsP4)[0].Pt() ) ;
	else 
          SFMuID = ratioMuIDEC->Eval( (*diTauLegsP4)[0].Pt() ) ;
	if(TMath::Abs((*diTauLegsP4)[0].Eta())<=0.8)
	  SFMuIso = ratioMuIsoBL1->Eval( (*diTauLegsP4)[0].Pt() ) ;
	else if(TMath::Abs((*diTauLegsP4)[0].Eta())>0.8 && TMath::Abs((*diTauLegsP4)[0].Eta())<=1.2)
	  SFMuIso = ratioMuIsoBL2->Eval( (*diTauLegsP4)[0].Pt() ) ;
	else
	  SFMuIso = ratioMuIsoEC->Eval( (*diTauLegsP4)[0].Pt() ) ; 
      }
      else{
	HLTMu  = 1.0;
	HLTTau = 1.0;
...
int main(int argc, const char* argv[])
{
  ...
  if ( argc==1 ) {
    ...
    makeTrees_MuTauStream("",           argv[1], atof(argv[2]), inputDir, dirOut, -1);
    
    if( string(argv[1]).find("Run2012-MuTau-All")!=string::npos )
      return 0;
    makeTrees_MuTauStream("TauUp",      argv[1], atof(argv[2]), inputDir, dirOut, -1);
    makeTrees_MuTauStream("TauDown",    argv[1], atof(argv[2]), inputDir, dirOut, -1);
    
    if( string(argv[1]).find("Embedded")!=string::npos)
      return 0;
    makeTrees_MuTauStream("JetUp",      argv[1], atof(argv[2]), inputDir, dirOut, -1);
    makeTrees_MuTauStream("JetDown",    argv[1], atof(argv[2]), inputDir, dirOut, -1);
  }
  else if( argc==7 ){
    makeTrees_MuTauStream(argv[1], argv[2], atof(argv[3]), argv[4], argv[5], (int)atof(argv[6]));
  }
  else {
