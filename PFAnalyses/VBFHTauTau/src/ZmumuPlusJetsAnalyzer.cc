...
ZmumuPlusJetsAnalyzer::~ZmumuPlusJetsAnalyzer(){
  delete jetsP4_; delete muonsP4_; delete jetsIDP4_; delete tagJetsIDP4_;
}
...
void ZmumuPlusJetsAnalyzer::analyze(const edm::Event & iEvent, const edm::EventSetup & iSetup){
  ...
  hltMu15v1_ = leg1->triggerObjectMatchesByPath("HLT_Mu15v1").size();
  hltIsoMu13v3_ = leg1->triggerObjectMatchesByPath("HLT_IsoMu13v3").size();
  hltIsoMu13v4_ = leg1->triggerObjectMatchesByPath("HLT_IsoMu13v4").size();
  ...
  for(unsigned int it = 0; it < jets->size() ; it++){
    ...
    sortedJets.insert( make_pair( (*jets)[it].p4().Pt()*scale ,scale*(*jets)[it].p4() ) );
  ...
  for(std::map<double, math::XYZTLorentzVectorD ,ZmumuPlusJetsAnalyzer::more>::iterator it = sortedJets.begin(); it != sortedJets.end() ; it++){
    jetsP4_->push_back( it->second );
  }
  for(std::map<double, math::XYZTLorentzVectorD ,ZmumuPlusJetsAnalyzer::more>::iterator it = sortedJetsID.begin(); it != sortedJetsID.end() ; it++){
    jetsIDP4_->push_back( it->second );
  }
  for(std::map<double, math::XYZTLorentzVectorD ,ZmumuPlusJetsAnalyzer::more>::iterator it = sortedTagJetsID.begin(); it != sortedTagJetsID.end() ; it++){
